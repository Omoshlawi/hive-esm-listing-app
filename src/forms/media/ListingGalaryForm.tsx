...
type ListingGalaryFormProps = {
  listingId: string;
  onClose?: () => void;
  onSuccess?: (media: Array<ListingMedia>) => void;
};

const FormSchema = z.object({
  media: ListingMediaSchema.omit({ url: true, metadata: true }).array(),
});
type FormData = z.infer<typeof FormSchema>;

const ListingGalaryForm: FC<ListingGalaryFormProps> = ({
  listingId,
  onClose,
  onSuccess,
}) => {
  ...
  const { addListingMedia, mutateListings } = useListingApi();
    ...
    try {
      ...
      const media = await Promise.allSettled(
        _files.map(async (f, i) => {
          ...
          return await addListingMedia(listingId, {
            mediaType: "IMAGE",
            url: f.path,
      ...
      <Stack justify="space-between">
        <Stack>
          <Dropzone
            ...
            onDrop={(f) => {
              setFiles(f);
              const _f = Array.from<ListingMediaFormData>({
                length: f.length,
...
export default ListingGalaryForm;
